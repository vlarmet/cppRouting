...
`cppRouting` is a `R` package which provide functions to calculate distances, shortest paths and isochrones/isodistances on weighted graphs. Under the hood, `cppRouting` call Dijkstra and A* algorithms implemented in C++ using std::priority_queue from the Standard Template Library.  
