...
#'  with \emph{SP} shortest distance/time, \emph{o} the origin node,
#' \emph{d} the destination node and \emph{t} the extra cost.
#'
...
get_detour <- function(Graph, from, to, extra=NULL, keep=NULL, long=FALSE) {
    ...
    if (is.null(extra)) {
        stop("No additional cost provided")
        }
    if (length(extra) != 1) {
        stop("extra should be one number")
        }
    extra <- as.numeric(extra)
    if (extra <= 0) {
        stop("extra must be positive")
        }
    ...
    res <- Detour(from_id, to_id, Graph$data$from, Graph$data$to,
                  Graph$data$dist, Graph$nbnode, t = extra, Graph$dict$ref, to_keep)
