...
get_isochrone<-function(Graph,from,lim,setdif=FALSE,keep=NULL,long=FALSE){
  ...
  if (length(lim)>1){
    lim<-sort(lim)
    maxlim<-max(lim)
    res<-Isochrone_multi(Graph$data$from,Graph$data$to,Graph$data$dist,Graph$nbnode,from_id,lim,maxlim,setdif,Graph$dict$ref,to_keep)
    ...
    res<-Isochrone(Graph$data$from,Graph$data$to,Graph$data$dist,Graph$nbnode,from_id,lim,Graph$dict$ref,to_keep)
    names(res)<-from
    if (long){
