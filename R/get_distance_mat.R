...
#' @param algorithm Character. Only for contracted graph, "mch" for Many to many CH,
#' "phast" for PHAST algorithm
#' @param allcores Logical. If TRUE, all cores are used.
#' @return Matrix of shortest distances.
#' @details If graph is not contracted, get_distance_matrix() recursively
#' perform Dijkstra algorithm for each 'from' nodes.
...
#' }
#'  See details in package website :
    ...
    if (length(Graph) == 4) {
        if (allcores == TRUE) {
            if (length(to) < length(from)) {
                res <- Dijkstra_mat_par(
                    to_id, from_id, Graph$data[, 2], Graph$data[, 1], Graph$data[, 3], Graph$nbnode)
            }
            else {
                res <- Dijkstra_mat_par(
                    from_id, to_id, Graph$data[, 1], Graph$data[, 2], Graph$data[, 3], Graph$nbnode)
            }
        } else {# allcores FALSE
            if (length(to) < length(from)) {
                res <- Dijkstra_mat(Graph$data[, 2], Graph$data[, 1], Graph$data[, 3],
                                    Graph$nbnode, to_id, from_id)
            } else {
                res <- Dijkstra_mat(Graph$data[, 1], Graph$data[, 2], Graph$data[, 3], Graph$nbnode,
                                    from_id, to_id)
            }
        }
    }
    if (length(Graph) == 5) {
        if (algorithm == "mch") {
            if (allcores == TRUE) {
                if (length(to) < length(from)) {
                    res <- par_Bidir_mat2(
                        to_id, from_id, Graph$data[, 2], Graph$data[, 1], Graph$data[, 3],
                        Graph$nbnode, Graph$rank)
                } else {
                    res <- par_Bidir_mat2(
                        from_id, to_id, Graph$data[, 1], Graph$data[, 2], Graph$data[, 3],
                        Graph$nbnode, Graph$rank)
                }
            } else {# allcores FALSE
                if (length(to) < length(from)) {
                    ...
                    res <- Bidir_mat3(from_id, to_id, Graph$data[, 1], Graph$data[, 2],
            ...
            if (allcores == TRUE) {
                if (length(to) < length(from)) {
                    res <- Phast_par(
                        invrank[to_id + 1], invrank[from_id + 1], invrank[Graph$data$to + 1],
                        invrank[Graph$data$from + 1], Graph$data[, 3], Graph$nbnode)
                } else {
                  res <- Phast_par(
                      invrank[from_id + 1], invrank[to_id + 1], invrank[Graph$data$from + 1],
                      invrank[Graph$data$to + 1], Graph$data[, 3], Graph$nbnode)
          }
            } else {# allcores FALSE
                if (length(to) < length(from)) {
                    res <- Phast3(
