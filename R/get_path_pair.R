...
#'
#'#' In A* and New Bidirectional A star algorithms, euclidean distance is used as heuristic
#' function.
...
get_path_pair <- function(Graph, from, to, algorithm = "bi", constant = 1,
            ...
            if (algorithm %in% c("NBA", "A*", "bi")) {
                if (algorithm == "A*") {
                    ...
                    res <- Astar_paths(
                        ...
                        from_id, to_id, Graph$coords[, 2], Graph$coords[, 3], constant,
                        Graph$dict$ref, to_keep)
                }
                if (algorithm == "NBA") {
                    message("Running NBA* ...")
                    res <- NBA_path(
                ...
                if (algorithm == "bi") {
                    ...
                    res <- Bidir_path(
                        from_id, to_id, Graph$data$from, Graph$data$to, Graph$data$dist,
                        Graph$nbnode, Graph$dict$ref, to_keep)
                }
            } else {# not in NBA A* bi
                message("Running Dijkstra ...")
                res <- Dijkstra_early_stop_path(
                    Graph$data$from, Graph$data$to, Graph$data$dist, Graph$nbnode,
                    from_id, to_id, Graph$dict$ref, to_keep)
            }
        }  else {
            if (algorithm == "bi") {
                message("Running bidirectional Dijkstra...")
    ...
    if (length(Graph) == 5) {
        ...
        res <- Bidir_mod_path(
            from_id, to_id, Graph$data$from, Graph$data$to, Graph$data$dist, Graph$nbnode,
            Graph$rank, Graph$dict$ref, to_keep, Graph$shortcuts$shortf, Graph$shortcuts$shortt,
