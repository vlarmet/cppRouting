...
#' @param directed logical. If FALSE, then all edges are duplicated by inverting
#' 'from' and 'to' nodes.
#' @param coords Optional. A data.frame or matrix containing all nodes coordinates.
#' Columns order should be 'node_ID', 'X', 'Y'.
...
#' 'cost' is a non-negative numeric vector describing the cost (e.g time, distance) between
#' each 'from' and 'to' nodes.
...
makegraph <- function(df, directed = TRUE, coords = NULL) {
    df <- as.data.frame(df)
    if (ncol(df) != 3) {
        stop("Data should have 3 columns")
    }
    ...
    if (any(df[, 3] < 0)) {
        stop("Negative cost is not allowed")
    }
    if (sum(df[, 1] == df[, 2]) > 0) {
        df <- df[df[, 1] != df[, 2], ]
    }
    ...
    if (!is.null(coords)) {
        ...
        if (sum(duplicated(coords[, 1])) > 0) {
            stop("Nodes should be unique in the coordinates data frame")
        }
        if (sum(Nodes %in% coords[, 1]) < length(Nodes)) {
    ...
    return(list(
        data = df, coords = coords, nbnode = length(Nodes), dict = dict)
        )
}
